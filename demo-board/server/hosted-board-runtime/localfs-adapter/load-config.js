...
function resolveRuntimeBoardsRegistry(config, tokens = {}, storageAdapter = 'localfs') {
...
export function loadLocalFsHostConfig(defaultConfigPath, cliArgs = process.argv.slice(2), processName = '') {
  ...
  return {
    ...
    taskExecutorTimeoutMs: Number.isFinite(Number(config.taskExecutorTimeoutMs)) ? Number(config.taskExecutorTimeoutMs) : undefined,
    chatAgentVisibilityMs: Number.isFinite(Number(config.chatAgentVisibilityMs)) ? Number(config.chatAgentVisibilityMs) : undefined,
    taskExecutorVisibilityMs: Number.isFinite(Number(config.taskExecutorVisibilityMs)) ? Number(config.taskExecutorVisibilityMs) : undefined,
    processAccumulatedVisibilityMs: Number.isFinite(Number(config.processAccumulatedVisibilityMs)) ? Number(config.processAccumulatedVisibilityMs) : undefined,
    queueConcurrency: Number.isFinite(Number(config.queueConcurrency)) ? Number(config.queueConcurrency) : undefined,
    queueFallbackPollMs: Number.isFinite(Number(config.queueFallbackPollMs)) ? Number(config.queueFallbackPollMs) : undefined,
    queueBoardRefreshMs: Number.isFinite(Number(config.queueBoardRefreshMs)) ? Number(config.queueBoardRefreshMs) : undefined,
    ...
    refsTemplates,
    uiTemplates,
    localfs: config.localfs && typeof config.localfs === 'object' && !Array.isArray(config.localfs) ? config.localfs : {},
