(function (global) {
  ...
  function buildManifestDocument(manifest, tools) {
    return {
      server: {
      ...
      tools: tools.map(function (tool) {
        return {
          ...
          inputSchema: tool.inputSchema && typeof tool.inputSchema === 'object'
            ? cloneJsonValue(tool.inputSchema)
  ...
  function createHttpRouteAgentfaceSurface(deps) {
    if (!deps || typeof deps !== 'object') {
      throw new Error('createHttpRouteAgentfaceSurface requires deps');
    }
    if (typeof deps.invokeHttpRoute !== 'function') {
      throw new Error('createHttpRouteAgentfaceSurface requires deps.invokeHttpRoute(method, path, bodyText)');
    }
    ...
    async function executeToolCall(name, args) {
      ...
      var remoteTool = resolveRemoteTool(tool);
      var safeArgs = args && typeof args === 'object' && !Array.isArray(args) ? cloneJsonValue(args) : {};
      ...
      if (routeKind === 'mcp-extras') {
        ...
        return toJsonToolResult(extrasPayload);
      }
      ...
      if (routeKind === 'mcp-raw') {
        return toRawToolResult(remoteTool, boardId, upstreamArgs, payload);
      }
      return toJsonToolResult(payload);
    }

    return {
      manifestDocument: buildManifestDocument(manifest, tools),
      listTools: function () {
  ...
  global.AgentfaceEmbeddedShared = {
    createHttpRouteAgentfaceSurface: createHttpRouteAgentfaceSurface,
